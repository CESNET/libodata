...
std::vector<T> XmlDocument::filterMap(
    ...
    std::function<T(const tinyxml2::XMLElement*)> map) const {
    std::vector<T> result;
    for (const auto& node : filter(doc.RootElement(), [&](const auto& element) {return element.Name() == node_name;})) {
...
std::vector<const tinyxml2::XMLElement*> XmlDocument::getEntries() const {
    return filter(doc.RootElement(), [](const auto& element) {return 0 == std::strcmp(element.Name(), "entry");});
}
...
std::vector<Product> XmlParser::parseList(
...
std::vector<std::string> XmlParser::parseManifest(
    ...
    std::function<std::string(
        const tinyxml2::XMLElement*)> map = [](auto node) {return node->Attribute("href");};
